...
export default function WorkspaceScreen() {
  ...
  // ── Redux State ──
  const { selectedWorkspace, selectedWorkspaceMembers, actionLoading, error } = useSelector((s: RootState) => s.workspace);
  const { workspaceDocuments, loading: docsLoading, actionLoading: docActionLoading } = useSelector((s: RootState) => s.document);
  ...
  const handleAddMember = async () => {
    ...
    const res = await dispatch(addWorkspaceMember({ workspaceId: workspaceId!, email: memberEmail.trim(), role: "viewer" }));
  ...
  const handleDeleteDoc = (docId: string) => {
    Alert.alert("Delete", "Are you sure?", [
      { text: "Cancel" },
      { text: "Delete", style: "destructive", onPress: () => dispatch(deleteDocument(docId)).then(() => dispatch(fetchWorkspaceDocuments(workspaceId!))) }
    ]);
  };
  ...
  return (
    <SafeAreaView className="flex-1 bg-white" edges={["top"]}>
      ...
      <View className="px-6 py-4 flex-row justify-between items-center">
        <TouchableOpacity onPress={() => nav.back()} className="w-10 h-10 items-center justify-center bg-gray-50 rounded-full">
          <Feather name="arrow-left" size={20} color="black" />
        </TouchableOpacity>
        <View className="bg-black px-4 py-1.5 rounded-full">
          <Text className="text-white text-[10px] font-black uppercase tracking-widest">{userRole}</Text>
        </View>
      </View>

      <ScrollView className="flex-1" showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 100 }}>
        <View className="px-6 mt-4">
          <Text className="text-[12px] font-black text-gray-300 uppercase tracking-[3px] mb-2">Workspace</Text>
          ...
          <Text className="text-gray-400 font-medium mt-3 text-base leading-6">
            {selectedWorkspace?.description || "A workspace for your brilliant ideas and shared documents."}
          </Text>

          {/* ── Stats Bar ── */}
          <View className="flex-row items-center gap-6 mt-8 py-6 border-y border-gray-50">
            <TouchableOpacity onPress={() => setShowMembers(!showMembers)} className="flex-row items-center gap-2">
              <View className="w-8 h-8 items-center justify-center bg-gray-50 rounded-lg">
                <Feather name="users" size={14} color="black" />
              </View>
              <Text className="text-sm font-black text-black">{selectedWorkspaceMembers.length} Members</Text>
            </TouchableOpacity>
            <View className="flex-row items-center gap-2">
              <View className="w-8 h-8 items-center justify-center bg-gray-50 rounded-lg">
                <Feather name="folder" size={14} color="black" />
              </View>
              <Text className="text-sm font-black text-black">{workspaceDocuments.length} Documents</Text>
            </View>
          </View>
          ...
          {isAdmin && (
            <View className="mt-8 bg-gray-50 p-6 rounded-[32px]">
              <Text className="text-[10px] font-black text-black uppercase tracking-widest mb-4">Add Contributor</Text>
              <View className="flex-row gap-2">
                ...
                />
                <TouchableOpacity onPress={handleAddMember} className="w-14 h-14 bg-black rounded-2xl items-center justify-center">
          ...
          <View className="mt-12 flex-row justify-between items-end mb-6">
            <View>
              <Text className="text-[12px] font-black text-gray-300 uppercase tracking-[3px] mb-1">Index</Text>
              <Text className="text-2xl font-black text-black">Documents.</Text>
            </View>
            {canEdit && (
              <TouchableOpacity 
                onPress={() => nav.push({ pathname: "/(workspace)/edit-doc", params: { workspaceId } })}
                className="bg-black px-5 py-2.5 rounded-2xl flex-row items-center gap-2"
              >
                <Feather name="plus" size={16} color="white" />
                <Text className="text-white text-xs font-black uppercase">Create</Text>
              </TouchableOpacity>
            )}
          </View>
          ...
          {docsLoading ? (
            ...
            <View className="gap-4">
              {workspaceDocuments.map((doc) => {
                ...
                return (
                  <View key={doc._id} className="bg-white border border-gray-100 rounded-[32px] overflow-hidden">
                    <TouchableOpacity 
                      activeOpacity={0.7}
                    ...
                    >
                      <View className="flex-1 pr-4">
                        ...
                        <Text className="text-xs font-bold text-gray-300 mt-1 uppercase tracking-widest">{doc.visibility}</Text>
                      </View>
                      <View 
                        style={{ width: 40, height: 40, borderRadius: 20, alignItems: 'center', justifyContent: 'center', backgroundColor: isOpen ? '#000' : '#F9FAFB' }}
                      >
                        <Feather name={isOpen ? "chevron-up" : "chevron-right"} size={18} color={isOpen ? "white" : "black"} />
                      </View>
                    </TouchableOpacity>
                      ...
                      <View className="px-6 pb-6 animate-in fade-in duration-300">
                        ...
                        {canEdit && (
                          <View className="flex-row gap-2 mt-4">
                            <TouchableOpacity 
                              onPress={() => nav.push({ pathname: "/(workspace)/edit-doc", params: { docId: doc._id, workspaceId, title: doc.title } })}
          ...
          {workspaceDocuments.length === 0 && !docsLoading && (
            <View className="py-20 items-center justify-center bg-gray-50 rounded-[40px]">
              ...
              <Text className="text-gray-400 font-black mt-4 uppercase tracking-widest">Empty Workspace</Text>
            </View>
          )}
        </View>
      </ScrollView>
